...
def sftp_conn():
    ...
    with pysftp.Connection(host='185.164.16.144', username='avis', password='*****', cnopts=cnOpts) as sftp1:
        print("Connected to sftp")
        sftp1.get_d(remotedir='/var/tmp/csv_files', localdir='./csvfiles', preserve_mtime=True)
    # sftp1.close()
    # for d in dir:
    #    if d[-4:] == '.csv':
    #        sftp1.get(d)
